...
void twi_init()
{
	...
	usart_msg("WELCOME");				//--- Send String to Com Port of PC
...
void usart_init()
{
	UBRR1H = 0;                                      // USART Baud Rate is set to 115200
	UBRR1L = 0x08;
	UCSR1C = (1 << UCSZ11) | (1 << UCSZ10);           // 8-Bit Data Selected
...
{
	while (!(UCSR1A & (1 << UDRE1)));                 // Check for Buffer is empty
	UDR1 = x;                                         // Send data to USART Buffer
}
...
{
	while (!(UCSR1A & (1 << RXC1)));                  // Check for data received completed
